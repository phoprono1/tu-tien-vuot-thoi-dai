...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log(`Character ${characterId} spent ${amount} spirit stones for: ${reason}`)

        return NextResponse.json({
            success: true,
            remainingStones: updatedCharacter.spiritStones,
            message: `Đã chi ${amount.toLocaleString()} linh thạch`
        })

    } catch (error) {
        console.error('Error spending spirit stones:', error)
